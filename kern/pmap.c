...
//	panic("mem_init: This function is not finished\n");
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, ROUNDUP(~0 - KERNBASE, PGSIZE), 0, PTE_W|PTE_P);
...
page_alloc(int alloc_flags)
{
	...
	struct PageInfo *alloc_pg = page_free_list;
	if(!alloc_pg) { 
		// No free mem.
		return NULL;
	} 
	// our function can only access mem via VA, so there needs a translation(PA->VA)
	if(alloc_flags & ALLOC_ZERO) {
		memset(page2kva(alloc_pg), '\0', PGSIZE);
	}
	page_free_list = page_free_list->pp_link;
	alloc_pg->pp_link = NULL;
	return alloc_pg;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uint32_t pd = (uint32_t)va, pu = ROUNDUP((uint32_t)va + len, PGSIZE);
	while (pd < pu) {
		pte_t *const pte = pgdir_walk(env->env_pgdir, (void *)pd, 0);
		if (!pte || (((uint32_t)(*pte) & (perm | PTE_P)) != (perm | PTE_P))) {
			user_mem_check_addr = pd;
			return -E_FAULT;
		}
		// little trick here for passing 'buggyhello2' test..
		pd = ROUNDDOWN(pd + PGSIZE, PGSIZE);
	}
	return 0;
}
